      ...
      <div className="mx-auto max-w-[1440px]">
        ...
        <div className="flex flex-col gap-6 sm:gap-8 max-w-3xl mx-auto">
          ...
          <motion.div
            ...
            className="flex flex-row items-center gap-5 sm:gap-8 rounded-2xl p-4 sm:p-6"
            style={{ background: "#fff", border: "1.5px solid #f0f0f0", boxShadow: "0 2px 16px rgba(0,0,0,0.05)" }}
          >
            <img src={abir.image} alt={abir.name} className="w-28 sm:w-40 lg:w-48 shrink-0 rounded-xl object-cover object-center" style={{ height: 180 }} />
            <div>
              <p className="text-[11px] sm:text-[13px] lg:text-[14px] text-[#444] leading-relaxed italic">&ldquo;{abir.quote}&rdquo;</p>
              <p className="mt-2 sm:mt-3 font-semibold text-[11px] sm:text-[13px] lg:text-[14px] text-[#0A0A0A]">{abir.name}</p>
          ...
          <motion.div
            initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true, amount: 0 }} transition={{ duration: 0.55, delay: 0.1 }}
            className="flex flex-row-reverse items-center gap-5 sm:gap-8 rounded-2xl p-4 sm:p-6"
            style={{ background: "#fff", border: "1.5px solid #f0f0f0", boxShadow: "0 2px 16px rgba(0,0,0,0.05)" }}
          >
            <img src={safwan.image} alt={safwan.name} className="w-28 sm:w-40 lg:w-48 shrink-0 rounded-xl object-cover object-top" style={{ height: 180 }} />
            <div>
              <p className="text-[11px] sm:text-[13px] lg:text-[14px] text-[#444] leading-relaxed italic">&ldquo;{safwan.quote}&rdquo;</p>
              <p className="mt-2 sm:mt-3 font-semibold text-[11px] sm:text-[13px] lg:text-[14px] text-[#0A0A0A]">{safwan.name}</p>
